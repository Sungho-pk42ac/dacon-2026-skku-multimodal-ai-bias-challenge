...
**ablation(범인 확정 ✅):** merged_v7_sft(콜드스타트만, GRPO 전) held-out = **0.4213**, merged_v7(+GRPO) = 0.4163.
→ **OOD 붕괴의 원인은 100% 콜드스타트 SFT** (GRPO 전 이미 0.42). GRPO는 무죄(0.42→0.42).
v4(0.836)→콜드스타트(0.421)→GRPO(0.416). "정답 박은 템플릿 + BBQ 100% SFT"의 파국적 망각.
= Chu 2025("SFT Memorizes, RL Generalizes") 예측 그대로 → **v8이 콜드스타트를 제거한 것이 정확한 처방**.
